package ramil.akberov.coursework3.controllers;
...
import ramil.akberov.coursework3.serviсes.FilesService;
import ramil.akberov.coursework3.serviсes.SocksService;
...
public class SocksController {
    ...
    public ResponseEntity<Integer> getSelectedSock(@RequestParam(name = "color") Colors color,
                                                   @RequestParam(name = "size") Size size,
                                                   @RequestParam(name = "cotton") Integer cotton,
                                                   @RequestParam(required = false) Integer quantity) throws ExceptionApp {
    ...
    public ResponseEntity<Socks> saleSocks(@RequestParam(name = "color") Colors color,
                                           @RequestParam(name = "size") Size size,
                                           @RequestParam(name = "cotton") Integer cotton,
                                           @RequestParam(required = false) Integer quantity) throws ExceptionApp {
        Socks saleSocks = socksService.saleSocks(color,size, cotton);
        if (saleSocks == null) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(saleSocks);
    }
    ...
    public ResponseEntity<Void> deleteDefectiveSockss(@RequestParam(name = "color") Colors color,
                                                      @RequestParam(name = "size") Size size,
                                                      @RequestParam(name = "cotton") Integer cotton,
                                                      @RequestParam(required = false) Integer quantity)throws ExceptionApp {
